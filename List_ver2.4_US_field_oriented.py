...
with open(f'{BASE_DIRECTORY}\\TEST_US.csv','r') as csv_file:
    ...
    for line in csv_reader:
        try: 
            ...
            # ------------------------------ Check Layout ------------------------------ #
            try:
                WebDriverWait(driver,2).until(EC.presence_of_element_located((By.XPATH,"//span[contains(text(),'You are currently viewing a new version of the Add')]")))
                newLayout = True
            ...
            try:
                if newLayout:
                    log('This product use new layout, no need to check for list price, skip...')     
                else: 
                    driver.find_element_by_xpath('//*[@id="list_price"]').send_keys(line[1])
